...
function enforceDepositLimit(balance){
  ...
  box.id=noticeId;box.className='card';
  ...
  document.querySelectorAll('.card').forEach(card=>{
    ...
    if(title.includes('official payment accounts')||title.includes('how to deposit')||title.includes('submit deposit request'))card.style.display='none';
  });
}
...
async function loadPaymentAccounts(){
  const {data,error}=await db.from("payment_settings").select("*").eq("is_active",true);
  if(error)return;
  data.forEach(item=>{
    if(item.method==="MTN"){document.getElementById("mtnName").textContent=item.account_name;document.getElementById("mtnNumber").textContent=item.phone_number;}
    if(item.method==="Airtel"){document.getElementById("airtelName").textContent=item.account_name;document.getElementById("airtelNumber").textContent=item.phone_number;}
  });
}
...
document.getElementById("copyMTN").onclick=()=>{navigator.clipboard.writeText(document.getElementById("mtnNumber").innerText);alert("MTN number copied.")};
document.getElementById("copyAirtel").onclick=()=>{navigator.clipboard.writeText(document.getElementById("airtelNumber").innerText);alert("Airtel number copied.")};
...
async function submitDeposit(){
  ...
  try{
    ...
    if(transaction===""){alert("Transaction ID is required.");return}
    if(transaction.length<6){alert("Transaction ID is too short.");return}
