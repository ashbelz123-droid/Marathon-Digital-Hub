...
function mdhStatus(text, error = false) {
  ...
  if (e) {
    e.textContent = text;
    e.className = 'admin-status ' + (error ? 'error' : 'ok');
  }
}

async function mdhRequest(token) {
  const { data, error } = await MDH_DB.functions.invoke(BRIDGE, {
    body: { action: 'users', page: 1, perPage: 1000 },
    headers: { Authorization: 'Bearer ' + token }
  });

  if (error) {
    const message = error.message || 'Unable to load users';
    const authError = /401|unauthorized|invalid.*session|expired/i.test(message);
    throw Object.assign(new Error(authError ? 'Unauthorized' : message), { auth: authError });
  }

  if (!data?.ok) {
    throw Object.assign(new Error(data?.error || 'Unable to load users'), {
      auth: /unauthorized|expired/i.test(data?.error || '')
    });
  }
...
async function mdhLoad(silent = false) {
  ...
  if (b) {
    b.disabled = true;
    b.textContent = silent ? 'Updating…' : 'Loading…';
  }
  if (!silent) mdhStatus('Loading all users…');

  try {
    const data = await mdhRequest(token);
    mdhUsers = data.users || [];
    mdhMachines = data.machines || [];
    mdhCards();
    mdhFilter();
...
function mdhCards() {
  ...
  [['totalUsers', mdhUsers.length], ['activeUsers', active], ['suspendedUsers', suspended], ['newUsers', fresh], ['walletTotal', mdhMoney(wallet)], ['machineOwners', owners], ['userCount', mdhUsers.length + ' Users']]
    .forEach(([id, v]) => { if (mdh$(id)) mdh$(id).textContent = v; });
}
...
function mdhRender(list) {
  ...
  if (!box) return;
  if (!list.length) {
    box.innerHTML = '<div class="emptyState">No users found.</div>';
    return;
  }

  box.innerHTML = list.map(u => {
    const machines = mdhMachines.filter(m => m.user_id === u.id);
    const photo = u.avatar_url
      ? '<img src="' + mdhEsc(u.avatar_url) + '" alt="User photo" class="avatar">'
      : '<div class="avatar">' + mdhEsc((u.fullname || 'U').charAt(0).toUpperCase()) + '</div>';
...
function mdhFilter() {
  const q = (mdh$('searchInput')?.value || '').toLowerCase();
  const s = mdh$('statusFilter')?.value || 'all';
  mdhRender(mdhUsers.filter(u =>
    (`${u.fullname || ''} ${u.phone || ''} ${u.email || ''} ${u.referral_code || ''}`).toLowerCase().includes(q) &&
    (s === 'all' || (u.account_status || 'active') === s)
  ));
}

document.addEventListener('DOMContentLoaded', () => {
  if (!localStorage.getItem('admin_bridge_token')) {
    location.href = 'admin-login.html';
    return;
  }
  ...
  mdh$('adminLogout')?.addEventListener('click', () => {
    clearInterval(mdhRefreshTimer);
    ['admin_bridge_token', 'admin_logged_in', 'admin_name', 'admin_role', 'admin_id'].forEach(k => localStorage.removeItem(k));
    location.href = 'admin-login.html';
  });
